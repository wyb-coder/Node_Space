...
def aggregate_predictions(
    ...
    per_model_times: List[Tuple[str, float]] = []
    with torch.no_grad():
        for loaded in models:
            start_ts = time.perf_counter()
            dataset = PredictionDataset(image_paths, loaded.transform)
            loader = DataLoader(
                dataset,
            ...
            for images, indices in progress:
                ...
                ensemble_logits[indices] += logits
            elapsed = time.perf_counter() - start_ts
            per_model_times.append((loaded.name, elapsed))
    ensemble_logits /= float(len(models))
    return ensemble_logits, per_model_times
...
def main() -> None:
    ...
    logits, per_model_times = aggregate_predictions(models, image_paths, args.batch_size, args.num_workers, device)
    ...
    for name, seconds in per_model_times:
        print(f" - {name}: {seconds:.2f} 秒")
