...
class BaseModel(models.Model):
    ...
    # 查询'gsmc'列中包含指定字符串的第一条记录
    def getbyGsmc(self, model, tstr):
        datas = model.objects.filter(gsmc=tstr).first()
        return model_to_dict(datas)

    # 查询yonghuName列中包含指定字符串的第一条记录
    def getbyYonghuName(self, model, tstr):
        datas = model.objects.filter(yonghuName=tstr).first()
        return model_to_dict(datas)
    ...
    # 根据用户名更新
    def updatebyusername(self, model, new_value, username):
        try:
            model.objects.filter(yonghuName=username).update(**new_value)
            return None
        except Exception as e:
            return "{}:{}".format(Exception, e)

    # 获取表中有多少条数据
    def getcount(self, model):
        return model.objects.all().count()

    # 向zhaopinxinxiRecomm表中插入一条数据
    def insertzhaopinxinxiRecomm(self, model, params):
        try:
    ...
    # 根据用户名删除
    def deletebyonghuname(self, model, username):
        try:
            model.objects.filter(yonghuName=username).delete()
            return None
        except Exception as e:
            return "{}:{}".format(Exception, e)
