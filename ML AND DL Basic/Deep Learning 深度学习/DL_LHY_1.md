...
3.*具体细节*：
  ...
  - *`nn.BatchNorm1d(output_dim)`*：对全连接层输出的每个特征维度做批量归一化，让训练更快、更稳、更容易收敛。
    ...
    - 令不同的*dimension*有相同的数据范围。

    - *Batch-Norm*`一次对一个`*`Batch`*`中的数据，做统一归一化`
      $$
      \vec{z}^i=\frac{z^i-\mu}{\sigma}
      $$

    - 显然的，归一化结果与同一*Batch*中`其他数据有关`，因此，需要随机打乱(*shuttle*)。当*Batch*足够大，能够代表(近似)对全部数据做统一归一化。

    - 缺陷：必须等待一整个*Batch*才能做到标准化。解决：在训练中途计算 $\vec{\mu}、\vec{\sigma}$，*Testing*时直接使用。


  - ==*concat_nframes*(帧拼接) *vs LSTM*(长短期记忆网络)：==
    - ==*concat_nframes*：==强制修改输入数据，将上下文(局部上下文)也作为输入。
    - ==*RNN*：循环神经网络==，也能使用上下文信息。每一步将当前输入和上一步的隐藏状态结合。
    - 在长序列中，梯度在反向传播时会逐渐衰减或爆炸，导致模型难以学习长期依赖。
...
- ==*Recurrent NN(RNN)*==：循环神经网络
- *==Long Short Term Memory(LSTM)==*
- 见[Part2: DL_LHY_2 4 Self-attention](D:\Study\CS Study\Note 2024-\ML AND DL Basic\Deep Learning 深度学习\DL_LHY_2.md)
...
1.*CNN*概述：专精于`图像处理`，同样可能适用于与图像处理存在共通的领域。
  ...
  - 需要是时刻注意的是，==任何时刻，输入*NN*都只能是二维的*Tensor*==，例如(*512, 80*)代表一次并行输入*512*个样本，而每个样本有80个特征。
    - 因此，当面对*Convolutional*后的，例如(*3, 512, 512*)的图片时，需要*Pooling*。一个完整的卷积流程大致如下：
      - ==> 假设输入图片尺寸为(*512, 512, 512, 3*)，即*512*张尺寸为(*512, 512*)的彩色图。
      - ==> *Convolutional*`卷积核本身就是三维的`，其会提取三个维度上的特征，并做*Sum*再加上一个*bias*，因此，`在这个过程中，图片本身也就被降为成为了”灰白图“`
      - \==> 经过卷积，尾层，这一批*batch*的尺寸变为(*512, 256，7，7*)，这里的==*256*代表卷积核的个数，也就是你想要在图片中提取到的特征数==。(*7, 7*)代表经过卷积，每一个卷积核最终过滤到了这一维的小图。
      - \==> ==GAP(全剧平均池化)，将(*7, 7*)的”小图“取平均，最终，每一个图片得到了*256*维的特征向量。==
        - *Pooling*(池化)的作用就是模糊位置信息，提取主要特征，减小计算量。

    - *`batch_size`*`在任何时刻，对网络结构与理解与实践都没有任何意义，其仅仅是一次并行的处理多少数据。`
...
2.*CNN*应用：围棋：考虑为*CLassification*，预测下一个棋子的“类别”
  ...
  - ==在懂原理的基础上，认识到某一部分带来的抽象/具体的影响，进一步自主的设计网络结构。例如*AlphaGo*不使用*Pooling*==
    - 不使用*Pooling*，就是`不接受池化对位置信息的模糊`。但又必须适配*NN*，因此，粗暴的强力*Flatten*。
...
3.具体细节：
  ...
  - *Cross Validation*(交叉验证)：把数据集划分成多个子集，轮流作为训练集和验证集，最终综合各次验证结果来评估模型性能。
    ...
    - *K-Fold*适用情况：数据量较小、追求评估的稳定性、竞赛刷分
...
4.任务指标与完成情况：
  ...
  - ![image-20251117091935878](D:\Study\CS Study\Note 2024-\All Photo\image-20251117091935878.png)
...
- 区分 $X$ 的行数与列数，行数代表一个*batch*(一次输入多少个样本)，`列数代表输入样本的特征维度(特征数目)`
- *nn.Linear(in_features, out_features)*
  - *in_features*：适应输入 $X$ 的特征数
  - *out_features*：`决定输出给下一层的特征维度，也就代表着本层的神经元个数，因为一个神经元仅仅输出一个样本。`
  - *Linear(8, 4)*，代表这一层有四个神经元，每个神经元输出*[32, 1]*，最终拼接为*[32, 4]*
  - 全部的神经元的样本数都是 $X_{row}$，也就是说，`全连接层每一层都能接收到全部信息。`
  - ==典型的全连接图中，变化的全部是特征维度信息==
    - 全连接层中，后面一层神经元向前连接着前面一层全部的神经元，代表着是列之间的拼接，而不是行之间的拼接,不是样本数的拼接。
- ==无论对于什么结构*NN、RNN*而言，样本数，$X$ 的行数，都是无关紧要的，完全取决于设定的*batch*==
